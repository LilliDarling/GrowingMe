...
async def create_post(post: PostIn, queries: PostQueries = Depends()):
    post_new = await queries.create_post(post=post)
    if post_new is None:
        raise HTTPException(
            status_code=400, detail="Post not created. Category may not exist."
        )
    return post_new
...
async def get_post(title=str, queries: PostQueries = Depends()):
...
async def update_post(
    patch: PostPatchSchema, title: str, queries: PostQueries = Depends()
):
...
async def delete_post(title: str, queries: PostQueries = Depends()):
